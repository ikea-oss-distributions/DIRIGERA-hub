...
#
# shellcheck disable=SC2010,SC2039,SC2046

set_boot_counter() {
    touch "/usr/share/persist/.should_verify_system"
    sync "/usr/share/persist/.should_verify_system"
...
write_fresh_metadata() {
    next_slot=$( test "$(get_cur_rauc_slot)" == 'A' && echo 'B' || echo 'A' )

    /usr/bin/fresh-metadata "$next_slot"
}
